#---------------------------------------GERADOR DE SENHA---------------------------------------
import PySimpleGUI as sg
import random as rd
import string as st



#Função para gerar a senha com base nas escolhas do usuário
def gerar_senha(quantidade, incluir_numero, inclur_maiuscula, incluir_minuscula, incluir_simbolo):
    caracters = ''

    if incluir_numero:
        caracters += st.digits
    if inclur_maiuscula:
        caracters += st.ascii_uppercase
    if incluir_minuscula:
        caracters += st.ascii_lowercase
    if incluir_simbolo:
        caracters += st.punctuation
    if not caracters:
        sg.popup_error('Selecione pelo menos um tipo de caracter para gerar sua senha.')
        return
    
    senha = ''.join(rd.choice(caracters) for _ in range(int(quantidade)))
    sg.popup(f'Senha gerada: {senha}', title= 'Senha Gerada')

#Fazer a interface
def tela_inicial():
    layout = [
        #Solicite ao usuário o comprimento desejado para a senha.
        [sg.Text('Digite a quantidade númerica para o tamanho da sua senha')],
        [sg.Input(key='quantidade')],
        #Pergunte ao usuário se ele deseja incluir números na senha (s/n).
        [sg.Text('Deseja Incluir números na sua senha?')],
        [sg.Radio('Sim', 'opcao_numero', key='sim_numero', default=False), sg.Radio('Não', 'opcao_numero', key='nao_numero')],
        #[sg.Radio('Incluir Número', 'opcao_numero',key='numero', default=False,)]
        #Pergunte ao usuário se ele deseja incluir letras maiúsculas na senha (s/n).
        [sg.Text('Desje Incluir letras Maiúsculas')],
        [sg.Radio('Sim', 'opcao_maiuscula', key='maiuscula', default=False), sg.Radio('Não', 'opcao_maiuscula', key='nao_maiuscula')],
        #Pergunte ao usuário se ele deseja incluir letras minúsculas na senha (s/n).
        [sg.Text('Deseja Incluir letras Minúsculas')],
        [sg.Radio('Sim', 'opcao_minuscula', key='minuscula', default=False), sg.Radio('Não', 'opcao_minuscula', key='nao_minuscula')],
        #Pergunte ao usuário se ele deseja incluir símbolos na senha (s/n).
        [sg.Text('Deseja Incluir letras Símbolos')],
        [sg.Radio('Sim', 'opcao_simbolo', key='simbolo', default=False), sg.Radio('Não', 'opcao_simbolo', key='nao_simbolo')],

        [sg.Button('Criar Senha')]
    ] 

    window = sg.Window('Gerador de Senhas', layout)

    while True:
        event, values = window.read()
        if event == sg.WIN_CLOSED:
            break
        if event == 'Criar Senha':
            quantidade = values['quantidade']
            incluir_numero = values ['sim_numero']
            incluir_maiuscula = values ['maiuscula']
            incluir_minuscula = values ['minuscula']
            incluir_simbolo = values ['simbolo']

            gerar_senha(quantidade, incluir_numero, incluir_maiuscula, incluir_minuscula, incluir_simbolo)

            window.close()
            tela_inicial()
            #tela de senha cadstrada
            


# Tela com senha gerada
tela_inicial()